...
import { ChevronLeft, FastForward, Rewind, SkipBack, SkipForward, Cpu } from "lucide-react";
import { useStockfish } from "@/hooks/useStockfish";

interface GameViewerProps {
  game: GMGame;
  startFen: string; // The 960 start position
  onBack: () => void;
}
...
export default function GameViewer({ game, startFen, onBack }: GameViewerProps) {
  ...
  const [gameChess, setGameChess] = useState<Chess | null>(null);
  const [currentMoveIndex, setCurrentMoveIndex] = useState(-1); // -1 = start
  const [history, setHistory] = useState<{ fen: string; san: string; move: any }[]>([]);
  ...
  useEffect(() => {
    ...
    // We try the main database first
  ...
  const findAndLoadGame = (dbText: string) => {
    // Simple strategy: Search for the block containing White and Black players
    // This is a heuristic but effective for this scale
    
    // Normalize names for searching
    const whiteSearch = `[White "${game.white}"]`;
    const blackSearch = `[Black "${game.black}"]`;
    
    // Split database into games (approximate by Event tag)
    const games = dbText.split('[Event "');
    
    let foundGamePgn = "";
    
    for (const g of games) {
      const fullPgn = '[Event "' + g; // Re-add the split token
      if (fullPgn.includes(whiteSearch) && fullPgn.includes(blackSearch)) {
        // Check date if needed, or assume matches are unique enough
        if (game.date && fullPgn.includes(game.date)) {
            foundGamePgn = fullPgn;
            break;
        }
        // Fallback match
        foundGamePgn = fullPgn; 
    ...
    if (foundGamePgn) {
      try {
        ...
        // Robust approach: Clean PGN move string and play on our known startFen
        const moveText = foundGamePgn.replace(/\[.*?\]/g, '').replace(/\n/g, ' ').replace(/\s+/g, ' ');
        // Remove result and move numbers
        const cleanMoves = moveText.replace(/(1-0|0-1|1\/2-1\/2|\*)$/, '');
        const tokens = cleanMoves.split(' ').filter(t => t && !/^\d+\./.test(t)); 
        
        const newHistory: { fen: string; san: string; move: any }[] = [];
        
        // Re-instantiate on startFen
        const game = Chess.fromSetup(setup).unwrap();
        ...
        for (const token of tokens) {
            ...
            try {
                const move = parseSan(game, token);
                if (move) {
                    const san = makeSan(game, move);
                    game.play(move);
                    newHistory.push({
                        fen: makeFen(game.toSetup()),
                        san: san,
                        move: move
                    });
                }
        ...
        setGameChess(Chess.fromSetup(setup).unwrap()); // Display state
        setCurrentMoveIndex(-1); // Start at beginning
        setLoading(false);

      } catch (e) {
        console.error(e);
        setError("Error parsing game.");
        setLoading(false);
      }
    } else {
        // Game metadata exists but PGN not found - show with starting position only
        try {
          const setup = parseFen(startFen).unwrap();
          setGameChess(Chess.fromSetup(setup).unwrap());
          setHistory([]);
  ...
  const currentDisplayFen = useMemo(() => {
    if (currentMoveIndex === -1) return startFen;
    return history[currentMoveIndex]?.fen || startFen;
  }, [currentMoveIndex, history, startFen]);
  ...
  // Colors for Move Table
  const getMoveColor = (evalScore: number) => {
      // Placeholder logic - real would come from engine
      return "text-creme";
  };
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className={`${mobileView === 'board' ? 'flex' : 'hidden'} sm:flex flex-1 justify-center bg-black/20 p-2 sm:p-4 min-w-0`}>
            <div className="w-full max-w-[500px] lg:max-w-[560px] aspect-square shadow-2xl shadow-black/50 my-auto">
                <ChessBoard fen={currentDisplayFen} arePiecesDraggable={false} />
            </div>
        </div>
        ...
        <div className={`${mobileView === 'moves' ? 'flex' : 'hidden'} sm:flex w-full sm:w-72 lg:w-96 flex-shrink-0 flex-col sm:border-l border-white/10 bg-surface overflow-hidden`}>
                ...
                {history.length > 0 ? (
                          ...
                          {Array.from({ length: Math.ceil(history.length / 2) }).map((_, i) => {
                              ...
                              return (
                                  <tr key={i} className="border-b border-white/5 hover:bg-white/5 transition-colors text-sm font-mono">
                                      <td className="py-1.5 pl-4 text-creme-muted/50 w-12">{moveNum}.</td>
                  ...
                  <div className="flex flex-col items-center justify-center h-full text-center p-6">
                    ...
                    <div className="text-creme-muted/30 text-xs max-w-[250px]">
                      The database contains game metadata but not full move notation. You can still view the starting position and use engine analysis.
                    </div>
                  </div>
                )}
            </div>
            ...
            <div className="absolute bottom-0 left-0 right-0 h-14 border-t border-white/10 flex items-center justify-center gap-4 bg-surface-dark z-20">
